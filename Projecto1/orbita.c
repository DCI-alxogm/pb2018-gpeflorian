...
int main (){
	...
	int M,T;
	double x0,y0,z0,vx0,vy0,vz0,X,Y,Z,VX,VY,VZ,r,G,m,h,i,count=0;
	...
	archivo=fopen("venus.txt","r");
		...
		fclose(archivo);
			...
			resultados=fopen("resultados_orbita.txt","w");
				...
				/*Se calcula la constante gravitacional en unidades astronómicas*/
				G=4.0*pow(3.141592653589,2);
				
				/*Se convierte la velocidad de unidades de tiempo días a años para empatar con las demás cifras que están en unidades atronómicas que serán usadas para los cálculos de la órbita*/
				vx0*=365.252
				vy0*=365.252
				vz0*=365.252
